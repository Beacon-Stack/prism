...
func init() {
	registry.Default.RegisterImportList("trakt_popular", func(_ json.RawMessage) (plugin.ImportList, error) {
		return &Plugin{}, nil
	})
	// No sanitizer needed — no secrets in settings (client comes from injection).
}

// Plugin fetches popular movies from Trakt. Zero-config: the Trakt client
// is injected by the service from the app-level Trakt client ID.
type Plugin struct {
	client *trakt.Client
}

func (p *Plugin) Name() string { return "Trakt Popular Movies" }
...
func (p *Plugin) Fetch(ctx context.Context) ([]plugin.ImportListItem, error) {
	...
	movies, err := p.client.GetPopular(ctx)
	if err != nil {
		return nil, err
	}
