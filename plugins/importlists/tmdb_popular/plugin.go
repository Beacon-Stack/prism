...
func init() {
	registry.Default.RegisterImportList("tmdb_popular", func(_ json.RawMessage) (plugin.ImportList, error) {
		return &Plugin{}, nil
	})
}

// Plugin fetches the TMDB popular movies list.
type Plugin struct {
	client *tmdb.Client
}

func (p *Plugin) Name() string { return "TMDB Popular Movies" }
...
func (p *Plugin) Fetch(ctx context.Context) ([]plugin.ImportListItem, error) {
	...
	results, err := p.client.GetPopularMovies(ctx, 1)
	if err != nil {
		return nil, err
	}
...
func (p *Plugin) Test(ctx context.Context) error {
	...
	_, err := p.client.GetPopularMovies(ctx, 1)
	return err
}
