...
func init() {
	registry.Default.RegisterImportList("trakt_box_office", func(_ json.RawMessage) (plugin.ImportList, error) {
		return &Plugin{}, nil
	})
}

// Plugin fetches current box office movies from Trakt.
type Plugin struct {
	client *trakt.Client
}

func (p *Plugin) Name() string { return "Trakt Box Office" }
...
func (p *Plugin) Fetch(ctx context.Context) ([]plugin.ImportListItem, error) {
	...
	movies, err := p.client.GetBoxOffice(ctx)
	if err != nil {
		return nil, err
	}
...
func (p *Plugin) Test(ctx context.Context) error {
	...
	_, err := p.client.GetBoxOffice(ctx)
	return err
}
