...
func TestSearch_WithTMDBID(t *testing.T) {
	...
	// First request: ?t=movie&tmdbid=27205
	...
	if p1.Has("cat") {
		t.Errorf("cat should not be set for movie search, got %q", p1.Get("cat"))
	}
	if got := p1.Get("apikey"); got != "testapikey" {
		t.Errorf("first request: expected apikey=testapikey, got %q", got)
	}

	// Second request: ?t=search&q=Inception+2010
...
// bug: some Torznab indexers silently ignore the `tmdbid` parameter and
// return the recent-uploads feed instead (since `?t=movie` with no params
// IS the recent feed). The fix is to ALWAYS run the text search as a
// safety net so the caller still gets the correct release.
//
// This test simulates an indexer that returns garbage for the tmdbid call
// and the right release for the text search. Both responses are merged via
// dedup-by-GUID, and downstream callers (autosearch, releases handler)
// then apply the title-match filter to drop the garbage.
...
<guid>https://indexer.example.com/release/garbage1</guid>
<link>https://indexer.example.com/download/garbage1.torrent</link>
<size>1000000</size>
</item>
</channel>
</rss>`
...
<guid>https://indexer.example.com/release/arrival1</guid>
<link>https://indexer.example.com/download/arrival1.torrent</link>
<size>2000000</size>
</item>
</channel>
</rss>`
	...
	idx, srv := newTestIndexer(t, func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/rss+xml")
		w.WriteHeader(http.StatusOK)
		// First call (tmdbid) → garbage. Second call (text) → arrival.
	...
	// Both releases must be returned (different GUIDs, no dedup collision).
	// The downstream title-match filter will drop the garbage one — that's
	// not this layer's job.
	if len(releases) != 2 {
		t.Fatalf("expected 2 releases (garbage + arrival), got %d", len(releases))
	}

	titles := []string{releases[0].Title, releases[1].Title}
	foundArrival := false
	for _, title := range titles {
		if strings.Contains(title, "Arrival") {
			foundArrival = true
			break
		}
	}
	if !foundArrival {
		t.Errorf("expected an Arrival release in results, got %v", titles)
	}
}
